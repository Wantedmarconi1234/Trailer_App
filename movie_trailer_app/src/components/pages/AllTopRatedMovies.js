...
const imageBaseUrl = 'https://image.tmdb.org/t/p/w500';
  ...
  useEffect( () => {
    const fetchData = async () => {
        try {
            const response = await fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${process.env.REACT_APP_API_KEY}&page=${pageCount}`);
  ...
  return (
    <section className="h-full my-5">
      ...
      <div className='flex items-center justify-center cursor-pointer'>
        <FaAnglesLeft  className={`${pageCount === 1 ? "hidden" : "block"} mx-2 text-2xl text-white hover:text-highlight-yellow`} onClick={decreaseCount}/>
